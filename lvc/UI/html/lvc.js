...
// ---------------- CVS LIGHTING CONTROLLER ----------------
// Receive state updates from Lua and reflect them on the panel.
window.addEventListener('message', function(event) {
	var d = event.data;
	if (d._type == 'cvs:setButtonState') {
		var el = document.querySelector('[data-btn="' + d.button + '"]');
		if (el) {
			if (d.state === true) {
				el.classList.remove('scene');
				el.classList.add('active');
			} else if (d.state === false || d.state === null) {
				el.classList.remove('active');
				el.classList.remove('scene');
			} else if (typeof d.state === 'string') {
				// Named CSS class — remove active and add the named class instead.
				el.classList.remove('active');
				el.classList.add(d.state);
			}
		}
	} else if (d._type == 'cvs:setDisplay') {
		var t = document.getElementById('cvs-display-text');
		if (t) { t.textContent = d.text; }
	} else if (d._type == 'cvs:setFooter') {
		var f = document.getElementById('cvs-footer');
		if (f) { f.textContent = d.text; }
	} else if (d._type == 'cvs:setPanel') {
		var p = document.getElementById('cvspanel');
		if (p) { p.style.display = d.visible ? 'flex' : 'none'; }
	}
});

// Forward button clicks to Lua.
$(function() {
	$('#cvspanel').on('click', '.cvs-btn, .cvs-display', function() {
		var btn = $(this).attr('data-btn');
		if (btn) {
			sendData('cvs:buttonClick', { button: btn });
		}
	});
});
