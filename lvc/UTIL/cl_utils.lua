...
function UTIL:GetToneAtPos(pos)
	if approved_tones == nil then return nil end
	if approved_tones[pos] ~= nil then
...
function UTIL:TogVehicleExtras(veh, extra_id, state, repair)
	...
	if type(extra_id) == 'table' then
		...
		elseif extra_id.add ~= nil and extra_id.remove ~= nil then
			...
			end		-- Plain array: { 4 } or { 4, 5 }
		else
			for _, id in ipairs(extra_id) do
				UTIL:TogVehicleExtras(veh, id, state, repair)
			end		end
	else
		if state then
			-- Always set the extra on, regardless of current state, to avoid
			-- stale reads from IsVehicleExtraTurnedOn after a same-frame turn-off.
			if repair then
				local doors = { }
				for i = 0,6 do
